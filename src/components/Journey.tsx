...
export default function Journey() {
  return (
    <section id="journey" className="relative py-24 lg:py-32 bg-bg-secondary overflow-hidden section-snap transition-colors duration-300">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <span className="text-blue-600 font-semibold text-sm tracking-widest uppercase">My path</span>
          <h2 className="text-3xl sm:text-4xl lg:text-[42px] font-bold font-[Space_Grotesk] text-text-primary mt-3">Education & Journey</h2>
        </motion.div>
        ...
        <div className="relative">
          ...
          {timeline.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: i % 2 === 0 ? -40 : 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className={`relative flex items-start mb-12 ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
              ...
              <div className="absolute left-4 md:left-1/2 w-4 h-4 rounded-full bg-gradient-to-r from-blue-400 to-blue-600 border-4 border-timeline-dot-border -translate-x-1/2 mt-6 z-10 shadow-lg shadow-blue-500/50 transition-all duration-300" />
              <div className={`ml-12 md:ml-0 md:w-1/2 ${i % 2 === 0 ? 'md:pl-12' : 'md:pr-12 md:text-right'}`}>
                <motion.div whileHover={{ scale: 1.02, y: -3 }} className="bg-card-bg border border-card-border shadow-md shadow-card-shadow rounded-xl p-5 sm:p-6 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300">
                  ...
                  <h3 className="text-lg font-bold text-text-primary font-[Space_Grotesk] mb-2">{item.title}</h3>
